...
for tool in awk dirname env find readlink; do type $tool > /dev/null 2>&1 || { echo -e "${cR}ERROR:${cZ} couldn't find '$tool' which is required by this script."; exit 1; }; done
...
if [[ "$CLANGFMT" == "clang-format" ]] && [[ -n "$COMSPEC" ]]; then # assume Windows
	...
	# Bash chokes on variable names with embedded parentheses
	PF86="$(cygpath -u "$(env|awk -F= '$1 == "ProgramFiles(x86)" {print $2}')")"
